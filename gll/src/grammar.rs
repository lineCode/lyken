...
impl<Pat: PartialEq> RuleWithNamedFields<Pat> {
    ...
    pub fn repeat(mut self) -> Self {
        ...
        self.rule = Rc::new(Rule::Repeat(self.rule));
        self
    }
}
...
impl<Pat: Ord + ToRustSrc> Rule<Pat> {
    ...
    fn field_is_refutable(&self, path: &[usize]) -> bool {
        match self {
            Rule::Empty | Rule::Match(_) | Rule::NotMatch(_) | Rule::Call(_) | Rule::Repeat(_) => {
                false
            }
...
pub macro grammar {
    ...
    (@rule [$name:ident : $input0:tt * $($input:tt)*] ($current:expr) [$($rules:expr)*]) => {
        grammar!(@rule [$($input)*] ($current.then(
            grammar!(@rule_tok $input0).repeat().with_field_name(stringify!($name))
        )) [$($rules)*])
    },
    (@rule [$input0:tt * $($input:tt)*] ($current:expr) [$($rules:expr)*]) => {
        grammar!(@rule [$($input)*] ($current.then(
            grammar!(@rule_tok $input0).repeat()
        )) [$($rules)*])
    },
    (@rule [$name:ident : $input0:tt $($input:tt)*] ($current:expr) [$($rules:expr)*]) => {
        grammar!(@rule [$($input)*] ($current.then(
...
impl<Pat: Ord + ToRustSrc> Rule<Pat> {
    fn generate_parse<'a>(
        ...
        Thunk::new(move |cont| match (&**self, parse_labels) {
            ...
            (Rule::Repeat(rule), None) => fix(|label| {
                parallel((rule.generate_parse(None) + call(label), thunk!("")))
            })(cont),
            (Rule::Repeat(rule), Some(parse_labels)) => fix(|label| {
                parallel((
                    thunk!("
                assert_eq!(_range.start(), c.frame.range.start());") +
                    rule.generate_parse(Some(parse_labels)) +
